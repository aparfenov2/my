...
public:
	...
	private:
		...
		bool adjust_lasti(s32 &lasti, const myvi::string_t spath, s32 spath_length) {
			...
			_MY_ASSERT(lasti < spath_length, return false); // путь не может заканчиватьс€ точкой
			_MY_ASSERT(spath[lasti] != '.', return false); // путь не может содержать несколько точек подр€д
...
public:
	...
	// об€зательно вызвать после инициализации кучи
	void init();
...
// интерфейс фабрики видов - выделен дл€ компилируемости
